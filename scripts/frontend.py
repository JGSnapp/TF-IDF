...
model_path = st.text_input(
    "Путь/ID модели (опционально)", value="../models/checkpoint-5613"
)
...
if st.button("Отправить"):
    with st.spinner("Отправляю запрос..."):
        try:
            r = requests.post(
                predict_url, json={"text": text, "model": model_path}, timeout=60
            )
            data = r.json()
...
for item in data:
    st.write(
        f"- Комментарий: {item['comment']}, Тип: {item['comment_type']}, Модель: {item['model']}"
    )
